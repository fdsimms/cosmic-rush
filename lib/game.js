(function () {
  ...
  var Game = SpaceInvaders.Game = function (shipImage, alienImage) {
    this.aliens = [];
    this.bullets = [];
    this.addAliens(alienImage);
    this.ship = new SpaceInvaders.Ship({ game: this, image: shipImage });
  };

  Game.prototype.addAliens = function (alienImage) {
    ...
    this.bullets.forEach(function (object1, idx1) {
      this.aliens.forEach(function (object2, idx2) {
        if (idx1 !== idx2) {
          ...
          object2.isCollidedWith(this.ship);
        }
      }.bind(this));
